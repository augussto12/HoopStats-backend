...
export const getMarketLock = async (req: any, res: any) => {
    ...
    `);
        ...
        // Comparación correcta (instantes reales)
        const now = new Date();

        const start = new Date(lock_start);
        const end = new Date(lock_end);

        const isLocked = !no_games_today && now >= start && now <= end;

        return res.json({
            isLocked,
            lockStart: start.toISOString(),
            lockEnd: end.toISOString(),
